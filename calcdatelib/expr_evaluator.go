...
func (n *OperationNode) Evaluate(ctx *EvalContext) (time.Time, error) {
	// Operations need a base date from context
	return time.Time{}, fmt.Errorf("operation node cannot be evaluated without a base date")
}
...
func (n *PipeNode) Evaluate(ctx *EvalContext) (time.Time, error) {
	...
	for _, op := range n.Operations {
		...
		default:
			return time.Time{}, fmt.Errorf("invalid operation in pipeline: %T", op)
		}
	}
...
func (n *RangeNode) Evaluate(ctx *EvalContext) (time.Time, error) {
	// Range nodes don't evaluate to a single time, they need special handling
	return time.Time{}, fmt.Errorf("range nodes must be handled separately")
}

// Evaluate evaluates a VariableNode
func (n *VariableNode) Evaluate(ctx *EvalContext) (time.Time, error) {
	if ctx.Variables == nil {
		return time.Time{}, fmt.Errorf("variable %s not found in context", n.Name)
	}
...
func (n *TransformNode) Evaluate(ctx *EvalContext) (time.Time, error) {
	// Transform nodes don't evaluate to a single time, they need special handling
	return time.Time{}, fmt.Errorf("transform nodes must be handled separately")
}

// EvaluateRange evaluates a range expression and returns start and end times
func EvaluateRange(node ExprNode, ctx *EvalContext) (start, end time.Time, err error) {
	rangeNode, ok := node.(*RangeNode)
	if !ok {
		return time.Time{}, time.Time{}, fmt.Errorf("not a range expression")
	}
...
func EvaluateTransform(transform *TransformNode, beginTime, endTime time.Time, index int, ctx *EvalContext) (newBegin, newEnd time.Time, err error) {
...
func EvaluateRangeExpression(input string, tz *time.Location) (start, end time.Time, err error) {
	parser := NewExprParser(input)
	node, err := parser.Parse(input)
	if err != nil {
